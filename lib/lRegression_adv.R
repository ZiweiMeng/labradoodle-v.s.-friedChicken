...
lRegression_adv<-function(fpath,k){
  ...
  sift_features<-read.csv("data/sift_features.csv",header = TRUE)
  sift_features<-data.frame(t(sift_features))
  n<-dim(sift_features)[1]
  sift_features$image<-seq(1:n)
  
  test_data<-read.csv("output/prediction_inceptionV3.csv")
  ...
  sift_features<-data.frame(t(sift_features))
  class<-read.csv("data/labels.csv")
  class<-unlist(class)
  ...
  logictRegression_cnn_featureSelect<-cv.glmnet(as.matrix(train_data[,1:k]),train_data$Class,alpha = 1,family = "binomial",type.measure='auc')

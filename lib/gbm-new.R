...
gbm_adv <- function(fpath,par=NULL){
  ...
  t1 <- Sys.time()
  gbm_fit <- gbm(label~ ., data = train_data[,1:k],distribution = "bernoulli",interaction.depth = depth, n.trees = n.trees, shrinkage = shrinkage,cv.folds = 5)
  t2 <- Sys.time() 
  t2 - t1
...
gbm_fit<-gbm_adv("cnn_features_350.csv")
# Get the estimation of n.trees
best.n.trees <- gbm.perf(gbm_fit,method = 'cv')
pred<-predict(gbm_fit,test_data[,1:n-1],n.trees=best.n.trees)>0
# Prediction accuracy
